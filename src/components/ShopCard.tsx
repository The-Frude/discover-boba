...
export default function ShopCard({ shop }: ShopCardProps) {
  ...
  const photo = shop.photos && shop.photos.length > 0 ? shop.photos[0] : '/images/boba-cat.jpeg'

  return (
    <div className={`card group ${isPremium ? 'border-2 border-yellow-400 dark:border-yellow-600' : ''}`}>
      <Link href={`/boba-shop/${shop.slug}`} className="block relative h-48 overflow-hidden bg-gray-100 dark:bg-gray-700">
        <OptimizedImage
          src={photo}
          alt={shop.name}
          fill
        ...
        />
        {isPremium && (
          <div className="absolute top-3 left-3 bg-yellow-400 dark:bg-yellow-600 text-white px-3 py-1 rounded-full text-xs font-bold shadow-sm">
            FEATURED
          </div>
        )}
        {hasRating && (

...
**Amended again 2026-09-09 (post-launch owner review):** the 60% bar above was applied to `extractTags()`'s flattened, substring-matched read of the `about` column - a narrow, partly-coincidental extraction (see docs/AUDIT.md's original LGBTQ+-friendly figure of "54% at best," which this correction supersedes). `about` is actually a structured JSON blob straight from the Google Places scrape (categories like "Crowd," "Dining options," "Offerings," each with real boolean attributes), and parsing it directly surfaces far more, and far better-populated, signal than the original extraction ever exposed - the true Chicago LGBTQ+-friendly figure is 27%, not 54%. A flat 60%-everywhere bar was throttling real, useful filters (LGBTQ+ friendly, family-friendly, vegan/vegetarian, women-owned, and others) down to noise. Replaced with a per-city floor of **≥5 shops and ≥3%** (`MIN_ATTRIBUTE_COUNT`/`MIN_ATTRIBUTE_PERCENT` in `src/utils/data.ts`), computed live against the real `about` JSON via `getAvailableAttributeGroups()` - loose enough to admit genuinely niche-but-real dimensions, floored so a filter never returns a near-empty result. `FILTER_ATTRIBUTES` in the same file is the full candidate registry (~20 dimensions across Access & service / Dietary / Good for / Inclusive & ownership); each city only ever renders the subset that clears its own floor - nothing is hand-curated per city. Social media presence is unrelated to `about` and keeps its original 60% threshold (Washington still excluded at 54%) since it was never the dimension in question. The table below is now illustrative, not authoritative - the real, current per-city list is whatever the live query returns; see `docs/AUDIT.md` §G for a snapshot at the time of this amendment.

**Per-city dimension table (from `docs/AUDIT.md`, recomputed per-city rather than by cross-city minimum) - superseded above for anything beyond these five plus social:**
...
Rating/reviews/hours clear 60% in all 7 cities individually and keep their original treatment. Delivery and Wheelchair accessible are now computed the same way as every other `about`-derived attribute (parsed live, not from the legacy `tags` column - see the Phase 5 rework amendment), which happens to correct a real, if minor, data discrepancy: the old `tags`-column-based Delivery count (738 sitewide) and the true `about`-derived count (637) disagreed by about 100 shops, because `tags` was a static snapshot computed once at migration time (via a function never actually called at read time) and never kept in sync.

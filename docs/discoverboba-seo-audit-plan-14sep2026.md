...
## Priority 1: Close the Content Depth Gap (highest impact) — ✅ Resolved 2026-09-14
...
**Status:** every active shop now has a `description_enriched` value. Of the 621 shops that lacked one, 105 were permanently-closed listings excluded on purpose (no point enriching a page for a shop that's gone), leaving 516 real targets — all 516 now enriched. Verified directly against Supabase: **680/680 active shops have `description_enriched`, 0 missing.**
...
- [ ] Cross-reference that list against Search Console's "Crawled – currently not indexed" and "Discovered – currently not indexed" URL lists (export both from GSC) — confirm the overlap. **Still not done - requires GSC access I don't have from this environment. Recommend exporting both URL lists from Search Console; now moot as a gate for the content work itself (already shipped) but still useful for confirming Priority 1 is what actually moves the indexation numbers in Priority 9's follow-up check.**
- [x] Extend the shop-level content generation pipeline (already built for the 271 - actually 192) to the remaining 621 shops, using the same staging-CSV-review QC pattern before committing to the live DB. **Done 2026-09-14.** Built `scripts/export-enrichment-batches.mjs` (grounds each shop in its real Google-sourced tags/attributes/rating/hours, excludes permanently-closed shops, splits into 16 per-city batch files) and dispatched one content-writing pass per batch (parallel background agents, one per ~20-40 shop batch) with explicit anti-hallucination rules: only state facts traceable to the provided data, no invented signature drinks/awards/press/opening dates/"family owned" claims. Reviewed a sample from 4 different cities directly before applying - specific, varied, honest about thin-data and non-boba businesses (e.g. a market that got swept into the dataset was described as a market, not assumed to be a tea shop). Applied via `scripts/apply-description-enrichment.mjs`, which only fills genuinely-empty rows so a re-run can never clobber another batch's or a human's edit. Result: 516/516 updated, 0 skipped, 0 errors.
- [ ] For shops lacking source description data entirely, define a fallback content strategy that's still unique per shop — e.g., combine category tags + neighborhood + hours + any review snippet data into a templated-but-varied paragraph, not a single boilerplate string. **Already exists independently of this plan: `generateShopBlurb()` in `src/utils/data.ts` does exactly this (deterministic per-shop opener + service/feature phrasing, seeded off the shop's own slug) and has been live since before this audit. Moot now in practice - every active shop has a real `description_enriched` value as of this pass, so this fallback tier should no longer be hit at all for active shops.**
- [x] Audit chain locations specifically (e.g., multiple "Kung Fu Tea" entries) — confirm each location's page has genuinely differentiated copy (neighborhood detail, hours, nearby landmarks), not the same paragraph with city swapped. **Checked both the original 4 enriched Kung Fu Tea locations and, in this pass, dense chain clusters (9 Happy Lemon locations in Seattle, 8 Gong Cha locations in NY, 4 Boba Heaven locations in Chicago) - each is written distinctly by its own neighborhood/street and varies its opener, not a single paragraph with city swapped.**
- [x] After each batch deployment, spot-check 5–10 pages live in production against what Claude Code reports as deployed (per the existing "verify in prod, not just in the report" practice). **Done 2026-09-14:** queried Supabase post-apply (680/680 active shops enriched) and curled 3 live shop pages across 3 different cities (Atlanta, Seattle, Washington) - each serves its new enriched description in the initial HTML, matching what was written to the DB.

**One data-quality issue surfaced during this pass, unrelated to content quality:** two shops tagged `city = 'Philadelphia'` have real addresses far outside the metro - `bobatea` (533 E 25th Ave, Altoona, PA - ~200mi west) and `tbubbles-green` (3700 Massillon Rd, Uniontown, OH - a different state entirely). Both got honest descriptions grounded in their real addresses rather than fabricated Philadelphia context, but the underlying `city` mislabeling (likely a source-data/geocoding error from the original import) means they're probably showing up on the Philadelphia listing page incorrectly. Worth a follow-up look, separate from this content push.
